...
namespace Fastener_Detection
{
    public class FastenerDetector
    {
        ...
        internal static int RemovalDirectionFinderUsingObb(TessellatedSolid solid, BoundingBox obb)
        {
            ...
            var equInDirections =
                Geometric_Reasoning.StartProcess.Directions.Where(d => Math.Abs(d.dotProduct(normalGuess) - 1) < 0.001).ToList()[0];
        ...
        internal static int RemovalDirectionFinderUsingObbWithTopPlane(TessellatedSolid solid, BoundingBox obb, out PolygonalFace topPlane)
        {
            ...
            var equInDirections =
                Geometric_Reasoning.StartProcess.Directions.First(d => Math.Abs(d.dotProduct(normalGuess) - 1) < 0.05);
            return Geometric_Reasoning.StartProcess.Directions.IndexOf(equInDirections);
        }
