...
namespace Plan_Generation
{
    internal class DBGBinary
    {
        ...
        private static Dictionary<hyperarc, List<hyperarc>> CombineWithNonAdjacentBlockingsUsingSecondaryConnections(
            ...
            bool relaxingSc)
        {
            var oppositeDir = new List<int> { StartProcess.DirectionsAndOppositsForGlobalpool[cndDirInd] };
        ...
        {
            if (dirs.Any(StartProcess.DirectionsAndSame[d].Contains)) return true;
            return false;
        }
        ...
        {
            var direction = StartProcess.Directions[cndDirInd];
