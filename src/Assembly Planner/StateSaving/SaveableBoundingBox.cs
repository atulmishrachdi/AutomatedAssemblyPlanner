...
namespace Assembly_Planner
{
    class SaveableBoundingBox
    {
        ...
        public BoundingBox generate()
        {
            ...
            result.Center = Center.generate();
            result.CornerVertices = (Vertex[])CornerVertices.Select((el, idx) => el.generate());
            result.Dimensions = Dimensions;
            result.Directions = Directions;
            result.PointsOnFaces = (List<Vertex>[])PointsOnFaces.Select((el, idx) => (el.Select((elem, indx) => elem.generate())));
