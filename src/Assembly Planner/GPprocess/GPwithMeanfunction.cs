...
namespace GPprocess
{
    public class GPwithMeanfunction
    {
        ...
        static void mMain(string[] args)
        {
            ...
            Readwholedata.getdata(address, actionname, out obdataX, out obdataY, out testpoints, out Ytrueb, out dim);
            dim++;//beta
            if (testy == "../../../../rotatedata/dataset1/TestY.csv")
            {
                dim = dim - 5;
...
///  ,double[] yture
)
        {
            ...
            GnuPlot.Plot(obdataX, obdataY, "title'Observations'with points pt 3" + (int)PointStyles.Dot + "lc rgb 'red' lw 1.5");
            GnuPlot.Plot(testpoints, mean, "title'Mean'with linespoints pt " + (int)PointStyles.Dot + "lc rgb 'blue' lw 1");
            ...
            GnuPlot.Plot(testpoints, XandUper, "title'95% CI'with linespoints pt " + (int)PointStyles.Dot + "lc rgb 'slategray' lw 1.5");
            GnuPlot.Plot(testpoints, XandLower, "title'Lower'with linespoints pt " + (int)PointStyles.Dot + "lc rgb 'slategray' lw 1.5");
        }
