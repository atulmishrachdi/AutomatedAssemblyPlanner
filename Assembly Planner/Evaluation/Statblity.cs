...
namespace AssemblyEvaluation
{
    class Statblity
    {
        public int NumOfMovalbeDire(SubAssembly newSubAsm, AssemblyCandidate c,List<TessellatedSolid> solids)
        {
            ...
            var ListOfmovingNodes = new List<node>();
            foreach(var p in newSubAsm.Install.Moving.PartNodes)
            {
                ListOfmovingNodes.Add(c.graph.nodes.Find(n => n.name.Equals(p)));
            }
            ...
            foreach (var ms in movablesolids)
            {
                ...
                var sss = msnode.arcs;
                var partmovingdire = DisassemblyDirectionsWithFastener.FreeLocalDirectionFinder(c.graph.nodes.Find(n=>n.name.Equals(ms.Name)), ListOfmovingNodes);
